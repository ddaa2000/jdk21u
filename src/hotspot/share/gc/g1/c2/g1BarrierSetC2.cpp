 ...
 */
 ...
 Node* G1BarrierSetC2::prefetch_load_barrier(GraphKit* kit,
                           Node* ctl,
                           Node* obj) const{
   ...
   __ if_then(obj, BoolTest::ne, kit->null()); {
     ...
     Node* load_count_new_val_masked = kit->gvn().transform(new AndXNode(load_count_new_val, __ ConX(0xFFFFF)));
 ...
 Node* G1BarrierSetC2::load_at_resolved(C2Access& access, const Type* val_type) const {
   ...
   if (on_weak || on_phantom) {
     ...
     return prefetch_load_barrier(kit, kit->control(), load);
   } else {
     return load;
   }
   // return load;

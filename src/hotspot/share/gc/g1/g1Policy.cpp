...
void G1Policy::record_young_collection_end(bool concurrent_operation_is_full_mark, bool evacuation_failure) {
  ...
  if (update_stats) {
    ...
    if (copied_bytes > 0) {
      ...
      log_info(gc)("[profile: cost_per_byte_ms] predict: %.12f, real: %.12f", _analytics->predict_zero_bounded(&_analytics->_cost_per_byte_copied_ms_seq, is_young_only_pause), cost_per_byte_ms);
